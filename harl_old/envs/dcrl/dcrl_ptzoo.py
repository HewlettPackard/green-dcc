...
class DCRLPettingZooEnv(ParallelEnv):
    def __init__(self, env_config):
        ...
        if env_config['partial_obs']:
            ...
            from envs.dcrl_env_harl_partialobs import DCRL  # pylint: disable=import-error,import-outside-toplevel
        else:
            if env_config['month'] == 1:
        ...
        if env_config['nonoverlapping_shared_obs_space']:
            ...
            self.share_observation_space = {agent: spaces.Box(low=-2.0, high=2.0, shape=(15,), dtype=np.float32) for agent in self.possible_agents}
        else:  
