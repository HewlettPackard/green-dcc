...
from HRL_multiple_rollouts_sequential.hierarchical_ppo import HierarchicalPPO as HRLPPO  # pylint: disable=C0413,E0611,E0001
from HRL_multiple_rollouts_sequential.greendcc_env import GreenDCC_Env  # pylint: disable=C0413
...
def worker_process(worker_id, child_conn, env_config, ppo_agent_params, num_cores_per_worker, experience_per_worker, assigned_cores):
    ...
    while True:
        ...
        if isinstance(message, tuple):
            command, training_phase = message
        else:
            command = message
            training_phase = 'joint'
            
        if command == 'collect_experience':
            experiences, stats, state = collect_experience(env, ppo_agent, env_config, state, experience_per_worker, training_phase)
...
def collect_experience(env, ppo_agent, env_config, state, experience_per_worker, training_phase='high_policy'):
    experiences = {
        'high_policy': {
            'states': [],
            'actions': [],
    ...
    while experience_counter < experience_per_worker:
        ...
        if ppo_agent.action_counter % ppo_agent.high_policy_action_freq == 0:
            # Set evaluate flag based on whether high_policy is being trained
            evaluate_high = (training_phase == 'low_policies')
            
            if evaluate_high:
                # In evaluation mode, only action is returned
                high_level_action = ppo_agent.high_policy.select_action(state['high_level_obs'], evaluate=True)
                high_level_logprobs = None
                high_level_state_values = None
                # print(f'Evaluating high-level policy - Action: {high_level_action}')

            else:
                # In training mode, action, logprobs, and state_values are returned
                high_level_action, high_level_logprobs, high_level_state_values = ppo_agent.high_policy.select_action(state['high_level_obs'], evaluate=False)
        ...
        for dc_id in ppo_agent.ll_policy_ids:
            ...
            # if training_phase == 'joint':
                # print(f'Worker {os.getpid()} - Training phase: {training_phase} - DC: {dc_id} - State LL: {state_ll}')
            # Set evaluate flag based on whether low_policies are being trained
            evaluate_low = (training_phase == 'high_policy')
            
            if evaluate_low:
                # In evaluation mode, only action is returned
                low_level_action = policy.select_action(state_ll, evaluate=True)
                low_level_logprobs = None
                low_level_state_values = None
                # print(f'Evaluating low-level policy for DC {dc_id} - Action: {low_level_action}')
            else:
                # In training mode, action, logprobs, and state_values are returned
                low_level_action, low_level_logprobs, low_level_state_values = policy.select_action(state_ll, evaluate=False)
                # Store experiences
    ...
    if not evaluate_high and experiences['high_policy']['rewards']:
    ...
    for dc_id in ppo_agent.ll_policy_ids:
        policy = ppo_agent.low_policies[dc_id]
        if not evaluate_low and experiences['low_policies'][dc_id]['rewards']:
...
def main():
    ...
    parser.add_argument('-c', '--core_offset', type=int, default=32, help='Core offset for CPU assignment')
    ...
    hl_lr_actor = 0.001       # learning rate for high level actor network
    hl_lr_critic = 0.003       # learning rate for high level critic network
    ll_lr_actor = 0.001       # learning rate for low level actor network(s)
    ll_lr_critic = 0.003       # learning rate for low level critic network(s)
    ...
    evaluation_interval = 500000  # Evaluate every 100,000 timesteps
    next_evaluation_step = evaluation_interval
    
    next_bias_plot_step = 10000
    bias_plot_freq = next_bias_plot_step

    # Record the start time
    benchmark_start_time = time.time()
    phase_length = 100000  # Number of episodes per phase (adjust as needed)

    while total_timesteps <= max_training_timesteps:
        # Collect experiences from all workers
        try:
            phase_cycle = (total_timesteps // phase_length) % 3

            if phase_cycle == 0:
                update_high_policy = True
                update_low_policies = False
                training_phase = 'high_policy'
            # elif phase_cycle == 1:
            #     update_high_policy = False
            #     update_low_policies = True
            #     training_phase = 'low_policies'
            # else:
            #     update_high_policy = True
            #     update_low_policies = True
            #     training_phase = 'joint'
            ...
            for conn in parent_conns:
                conn.send(('collect_experience', training_phase))
            ...
            # Update policies based on the current timestep
            # Determine which policies to update
            
            # Update policies
            high_policy_loss, low_policy_losses = global_ppo_agent.update_with_experiences(
                aggregated_experiences,
                update_high_policy=update_high_policy,
                update_low_policies=update_low_policies
            )
            ...
            if high_policy_loss is not None:
            ...
            if low_policy_losses is not None:
                for i, dc_id in enumerate(global_ppo_agent.ll_policy_ids):
            ...
            # plot the last layer parameter of the actor network of the low level policy (discrete action space, 5 output neurons)
            if total_timesteps >= next_bias_plot_step:
                for i, dc_id in enumerate(global_ppo_agent.ll_policy_ids):
                    actor_weights = global_ppo_agent.low_policies[dc_id].actor.actor[-2].bias.detach().numpy()
                    writer.add_histogram(f'ActorWeights/DC_{i+1}', actor_weights, total_timesteps)
                next_bias_plot_step += bias_plot_freq
