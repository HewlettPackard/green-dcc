...
DEFAULT_RL_CHECKPOINT_PATH = "checkpoints/train_20250505_120803/best_checkpoint.pth" # ADJUST THIS

EVALUATION_DURATION_DAYS = 7 # Use 1 month for evaluation
NUM_SEEDS = 5 # Number of random seeds to run for each controller
...
def run_single_evaluation(controller_config, seed, base_sim_cfg, base_dc_cfg, base_reward_cfg):
    ...
    for step_info in all_infos:
        ...
        for dc_name, dc_info in step_info.get("datacenter_infos", {}).items():
            ...
            total_water_usage += dc_info.get("agent_dc", {}).get("dc_water_usage", 0.0) # Check key name
    ...
    results = {
        ...
        "Total Water Usage (L)": total_water_usage, # Assuming units are Liters
...
if __name__ == "__main__":
    ...
    all_results = []
    for controller_config in CONTROLLERS_TO_EVALUATE:
        seed_results = []
        for seed in SEEDS:
            result = run_single_evaluation(
                controller_config, seed, base_sim_config, base_dc_config, base_reward_config
            )
            if result:
                seed_results.append(result)
            else:
                logger.warning(f"Skipping results for {controller_config['name']} seed {seed} due to error.")

        if seed_results:
            all_results.extend(seed_results)
        else:
             logger.warning(f"No successful runs for controller: {controller_config['name']}")


    if not all_results:
        logger.error("No evaluation results collected. Exiting.")
        exit()
    ...
    summary_df = results_df.groupby("Controller").agg(
        {
            ...
            "Total Water Usage (L)": ['mean', 'std'], # <<<--- ADDED
            "Average PUE": ['mean', 'std'],          # <<<--- ADDED
            "Total Tasks Deferred": ['mean', 'std'],   # <<<--- ADDED
        }
    )

    # Flatten multi-index columns and format
    ...
    for col in summary_df.columns:
        if col != "Controller":
            if 'mean' in col:
                formatted_summary_df[col] = summary_df[col].map('{:.2f}'.format)
            elif 'std' in col:
                 mean_col = col.replace('_std', '_mean')
                 new_col_name = col.replace('_std', '')
                 # Handle potential missing mean col if std exists but mean doesn't (shouldn't happen with agg)
                 if mean_col in formatted_summary_df.columns:
                      formatted_summary_df[new_col_name] = formatted_summary_df[mean_col] + \
                                                           ' ± (' + summary_df[col].map('{:.2f}'.format) + ')'
                      formatted_summary_df = formatted_summary_df.drop(columns=[mean_col]) # Drop original mean col
                 formatted_summary_df = formatted_summary_df.drop(columns=[col]) # Drop original std col
