    ...
    with tqdm(total=max_iterations, ncols=150) as pbar:
        while not done:
            if strategy_id == 0:  # PPO Agent
                actions = {
                    "DC1_ls_policy": np.clip(policies["DC1_ls_policy"].compute_single_action(obs["DC1_ls_policy"], explore=False)[0], -1, 1),
                    "DC2_ls_policy": np.clip(policies["DC2_ls_policy"].compute_single_action(obs["DC2_ls_policy"], explore=False)[0], -1, 1),
                    "DC3_ls_policy": np.clip(policies["DC3_ls_policy"].compute_single_action(obs["DC3_ls_policy"], explore=False)[0], -1, 1),
                }
