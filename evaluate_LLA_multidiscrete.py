...
#%%
# Load the checkpoint
FOLDER = 'results/LLAPPO/MultiDiscrete/PPO_LLADCRL_d1385_00000_0_2024-09-20_23-55-30'
    ...
    with tqdm(total=max_iterations, ncols=150) as pbar:
        while not done:
            if strategy_id == 0:  # PPO Agent
                ...
                actions = {
                    "DC1_ls_policy": np.array([2, 5], dtype=np.float32),
                    "DC2_ls_policy": np.array([2, 5], dtype=np.float32),
                    "DC3_ls_policy": np.array([2, 5], dtype=np.float32),
                }
