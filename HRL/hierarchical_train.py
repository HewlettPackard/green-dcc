...
async  def main():
    ...
    while time_step <= max_training_timesteps:
        ...
        for _ in range(1, max_ep_len+1):
            ...
            # saving reward and is_terminals here # states, actions, logprobs and state_values are added internally in select_action
            # high level policy changes have to be executed at lower frequency
            ...
            if (time_step % save_model_freq) == 0:
                if sum(print_avg_reward) > best_reward:
                    print("--------------------------------------------------------------------------------------------")
                    ...
                    print("--------------------------------------------------------------------------------------------")
                    best_reward = sum(print_avg_reward)
        ...
        print_running_reward = [i+j for i,j in zip(print_running_reward, [hl_current_ep_reward] + ll_current_ep_reward)]
        print_running_episodes += 1

        log_running_reward = [i+j for i,j in zip(print_running_reward, [hl_current_ep_reward] + ll_current_ep_reward)]
